...
void APP_COMM_SetBroadcastFrame(APP_frame_t *app_f, APP_frame_t *broad_f)
{
  ...
  broad_f->type = app_f->type;
  broad_f->address = app_f->address;
  broad_f->group = app_f->group;
  broad_f->len = app_f->len;
  for(i = 0; i < app_f->len; i++)
    broad_f->buffer[i] = app_f->buffer[i];
}
  ...
  switch (ApplicationStatus)
  {
    ...
    case APP_IDLE:
      ...
      if (nNStatus.operation == N_DOING)
      {
        ...
        if (APP_COMM_FrameReceived(&APP_Frame))
        {
          /* Verify if the received data from the COMM channel are addressed to the device or broadcast, 
             otherwise will be forwarded through PL */
          isBroadcasted = APP_Frame.type & APP_BROADCAST;
          if (APP_IsDeviceAddress(&APP_Frame) || ((APP_Frame.type & (~APP_BROADCAST)) == APP_PROGRAMMING_FRAME) || isBroadcasted)
          {
            APP_COMM_SetBroadcastFrame(&APP_Frame, &APP_Broadcast_Frame);
            ...
            //If a frame is broadcasted whether it is addressed on this device or not
            //Forwarding it throughout the PL
            if(isBroadcasted)
            {
              ApplicationStatus = APP_CONN_REQUEST;
